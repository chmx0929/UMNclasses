...
extern byte_t address_cal; //definitions by Kartik for LEAL
extern int NO_LEAL;
extern word_t aluC;
...
int instr_limit = 100000; /* Instruction limit [TTY only] (-l) */
...
void do_id_wb_stages()
{
   ...
   /*Calculate LEAL address */ //changes by Kartik
   //byte_t lealfun = gen_lealfun();
   //if (lealfun != NO_LEAL){  //lealfun is the icode, it sets the amount of shift
   //  address_cal = d_regvalb*(lealfun) + d_regvala;
   //  id_ex_next->valb = address_cal;
   //}
...
void do_ex_stage()
{
    ...
    word_t alua, alub ; // aluc; //changes by Kartik -- aluC defined as an extern
