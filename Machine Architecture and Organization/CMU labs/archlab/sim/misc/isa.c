...
extern int gui_mode;
alu_t aluC; //definition for leal
...
instr_t instruction_set[] = 
{
    ...
    {"leal0", HPACK(I_LEAL, LEAL0), 6, L_ARG, 1, 4, NO_ARG, 0, 0 },
    {"leal1", HPACK(I_LEAL, LEAL1), 6, L_ARG, 1, 4, NO_ARG, 0, 0 },
    {"leal2", HPACK(I_LEAL, LEAL2), 6, L_ARG, 1, 4, NO_ARG, 0, 0 },
    {"leal4", HPACK(I_LEAL, LEAL4), 6, L_ARG, 1, 4, NO_ARG, 0, 0 },
    {"leal8", HPACK(I_LEAL, LEAL8), 6, L_ARG, 1, 4, NO_ARG, 0, 0 },
...
// TODO     Modify this function to perform LEAL instructions
word_t compute_alu(alu_t op, word_t argA, word_t argB)
{
    ...
    switch(op) {
        ...
        case A_XOR:
            ...
            break;
        case LEAL0:
            val=argA+argB*0+aluC;
            break;
        case LEAL1:
            val=argA+argB*1+aluC;
            break;
        case LEAL2:
            val=argA+argB*2+aluC;
            break;
        case LEAL4:
            val=argA+argB*4+aluC;
            break;
        case LEAL8:
            val=argA+argB*8+aluC;
            break;
        default:
            break;
    }
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVL || hi0 == I_ALU || hi0 == I_PUSHL ||
	 ...
	 hi0 == I_MRMOVL || hi0 == I_IADDL || hi0 == I_LEAL ); //LEAL implementation
	...
	(hi0 == I_IRMOVL || hi0 == I_RMMOVL || hi0 == I_MRMOVL ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_IADDL || hi0 == I_LEAL ); //LEAL implementation
	...
	if (!reg_valid(hi1)) {
        ...
        word_t loVal, hiVal; loVal = 0;
	if (reg_valid(lo1)) loVal = get_reg_val(s->r, lo1);
        hiVal = get_reg_val(s->r,hi1);
        int factor =1;
        word_t setVal;
        aluC = cval;
        switch(lo0){
           case 0xF: factor = 0xF;
	           //s->cc = compute_cc(LEAL0, hiVal, loVal);
              break;
           case 0: factor = 0;
	           //s->cc = compute_cc(LEAL1, hiVal, loVal);
              break;
           case 1: factor = 1;
	           //s->cc = compute_cc(LEAL2, hiVal, loVal);
              break;
           case 2: factor = 2;
	           //s->cc = compute_cc(LEAL4, hiVal, loVal);
              break;
           case 3: factor = 3;
	           //s->cc = compute_cc(LEAL8, hiVal, loVal);
              break;
           default:
              break;
        }
        ...
        if (factor != 0xF) {
            setVal = cval + (loVal << factor) + hiVal;
        } else {
            setVal = cval + hiVal;
        }
	set_reg_val(s->r, lo1, setVal);
	// cval += get_reg_val(s->r, lo1);
	//if (!get_word_val(s->m, cval, &val))
	//    return STAT_ADR;
	s->pc = ftpc;
