...
{
    //fprintf(stderr,"Inside the get mem block");
    char rval = REG_NONE;
    int val = 0;
    int i;
    char c;
	...
	if (tokens[tpos].cval == '(') {
	    ...
	    if (tokens[tpos].type == TOK_REG){
		rval = find_register(tokens[tpos++].sval);
                //fprintf(stderr,"First reg in get mem is %d",rval);
            }
	    else {
		fail("Expecting Register Id");
		return;
...
void get_leal(int codepos)
{
   ...
   /* Deal with displacement */
    if (type == TOK_NUM) {
	val = tokens[tpos++].ival;
        //fprintf(stderr,"First val is %d",val);
	type = tokens[tpos].type;
    }else if (type == TOK_IDENT) {
	val = find_symbol(tokens[tpos++].sval);
	type = tokens[tpos].type;    
    }else{
        fail ("Expecting a number");
        return;
    }
	...
	if (tokens[tpos].cval == '(') {
	    tpos++;
            if (tokens[tpos].type == TOK_REG){
		rval1 = find_register(tokens[tpos++].sval);
                //fprintf(stderr,"First reg is %d",rval1);
            }
	    else {
		fail("Expecting Register Id");
		return;
