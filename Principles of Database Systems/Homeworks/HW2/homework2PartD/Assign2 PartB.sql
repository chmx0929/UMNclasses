...
3.							 
	SELECT C.cnames
	FROM Customer C, 
	WHERE C.cid NOT IN (SELECT B.cid
						FROM Buys B
						WHERE B.pid NOT IN (SELECT P.pid
											 FROM Product P
											 WHERE NOT EXISTS (SELECT C1.cid
											 				   FROM Customer C1
											 				   WHERE NOT EXISTS (SELECT B1.cid
															   					 FROM Buy B1
															  					 WHERE B1.pid = C1.pid AND C.cid = B.cid)));
4.
	SELECT A.name
	FROM Actors A, 
