...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	if (FIFO) {
		/**************************FIFO**************************/
		for (StrategyControl->nextVictimBuffer = 0; ;StrategyControl->nextVictimBuffer++)
		{
			printf("FIFO");
			...
			// If the buffer is pinned, we cannot use it so keep scanning the next buffer.
			LockBufHdr(buf);
			if (buf->refcount == 0)
			{
