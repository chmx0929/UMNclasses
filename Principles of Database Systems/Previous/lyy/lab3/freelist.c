...
//define a replace policy control type, 0: using FIFO, 1: using clock sweep
#define REPLACE_POLICY 0
...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	if(REPLACE_POLICY==0)
	{
		printf("you are using FIFO!");
		
		//initiate next buffer to consider grabbing to zero
		StrategyControl->nextVictimBuffer=0;
		
		//from the first element, find the first available buffer
		...
		//scan the whole buffer pool, no buffer available
		if(StrategyControl->nextVictimBuffer >= NBuffers)
		{
			StrategyControl->completePasses++;
			elog(ERROR,"no buffer available!");
		}
	}
	...
	if(REPLACE_POLICY==1)
	{
		...
		for (;;)
		{
			printf("you are using clock sweep!");
			buf = &BufferDescriptors[StrategyControl->nextVictimBuffer];
